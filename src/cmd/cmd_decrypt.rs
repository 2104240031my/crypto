...
pub fn cmd_main(args: Vec<String>) {
    ...
    let Ok(ct) = SuffixedArg::to_bytes(args[4].as_str()).map_err(printerrln) else { return; };

    let mut pt: Vec<u8> = Vec::<u8>::with_capacity(ct.len());
    unsafe { pt.set_len(ct.len()); }

    let mut ciph: Cipher = Cipher::new(algo, &key).unwrap();
    ciph.decrypt(&mut iv, &ct, &mut pt).unwrap();

    let Ok((pt_fmt, pt_src)) = SuffixedArg::parse_arg(args[5].as_str()).map_err(printerrln) else { return; };
    match pt_fmt {

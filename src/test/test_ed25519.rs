...
fn test_ed25519_inner(k: &[u8], a: &[u8], m: &[u8], s: &[u8]) {
    ...
    if !eqbytes(&a[..], &out1[..]) {
        println!("Err: testing Ed25519 is failed.");
        printbytesln(&a[..]);
        printbytesln(&out1[..]);
    }
    ...
    if !eqbytes(&s[..], &out2[..]) {
        println!("Err: testing Ed5519 is failed.");
        printbytesln(&s[..]);
        printbytesln(&out2[..]);
    }
    ...
    if let Err(e) = Ed25519::verify_oneshot(a, m, s) {
        match e.err_code() {
            CryptoErrorCode::VerifFailed => {
                println!("Err: testing Ed5519 is failed.");
                println!("Ed25519 Verification Failed.");
            },
            _ => panic!()
        }
    }

...
use crate::test::{
    DEBUG_PRINT_HMAC_SHA2,
    printbytesln,
    eqbytes
};

pub fn test_hmac_sha2() -> usize {
    let mut err: usize = 0;
    err = err + test_hmac_sha224();
    err = err + test_hmac_sha256();
    err = err + test_hmac_sha384();
    err = err + test_hmac_sha512();
    return err;
}
...
fn test_hmac_sha224_inner(k: &[u8], m: &[u8], d: &[u8]) -> usize {
...
fn test_hmac_sha256_inner(k: &[u8], m: &[u8], d: &[u8]) -> usize {
...
fn test_hmac_sha384_inner(k: &[u8], m: &[u8], d: &[u8]) -> usize {
...
fn test_hmac_sha512_inner(k: &[u8], m: &[u8], d: &[u8]) -> usize {
...
fn test_hmac_sha224() -> usize {
    ...
    err = err + test_hmac_sha224_inner(&K[..64], "Sample message for keylen=blocklen".as_bytes(), &d1[..]);
    ...
    err = err + test_hmac_sha224_inner(&K[..28], "Sample message for keylen<blocklen".as_bytes(), &d2[..]);
    ...
    err = err + test_hmac_sha224_inner(&K[..100], "Sample message for keylen=blocklen".as_bytes(), &d3[..]);

    return err;

}

fn test_hmac_sha256() -> usize {
    ...
    err = err + test_hmac_sha256_inner(&K[..64], "Sample message for keylen=blocklen".as_bytes(), &d1[..]);
    ...
    err = err + test_hmac_sha256_inner(&K[..32], "Sample message for keylen<blocklen".as_bytes(), &d2[..]);
    ...
    err = err + test_hmac_sha256_inner(&K[..100], "Sample message for keylen=blocklen".as_bytes(), &d3[..]);

    return err;

}

fn test_hmac_sha384() -> usize {
    ...
    err = err + test_hmac_sha384_inner(&K[..128], "Sample message for keylen=blocklen".as_bytes(), &d1[..]);
    ...
    err = err + test_hmac_sha384_inner(&K[..48], "Sample message for keylen<blocklen".as_bytes(), &d2[..]);
    ...
    err = err + test_hmac_sha384_inner(&K[..], "Sample message for keylen=blocklen".as_bytes(), &d3[..]);

    return err;

}

fn test_hmac_sha512() -> usize {
    ...
    err = err + test_hmac_sha512_inner(&K[..128], "Sample message for keylen=blocklen".as_bytes(), &d1[..]);
    ...
    err = err + test_hmac_sha512_inner(&K[..64], "Sample message for keylen<blocklen".as_bytes(), &d2[..]);
    ...
    err = err + test_hmac_sha512_inner(&K[..], "Sample message for keylen=blocklen".as_bytes(), &d3[..]);

...
fn test_sha224_inner(m: &[u8], d: &[u8]) {

    let mut out: [u8; 28] = [0; 28];

    Sha224::digest_oneshot(m, &mut out[..]).unwrap();
    if !eqbytes(d, &out[..]) {
        println!("Err: testing SHA-224 is failed.");
        printbytesln(&d[..]);
        printbytesln(&out[..]);
    }
...
fn test_sha256_inner(m: &[u8], d: &[u8]) {

    let mut out: [u8; 32] = [0; 32];

    Sha256::digest_oneshot(m, &mut out[..]).unwrap();
    if !eqbytes(d, &out[..]) {
        println!("Err: testing SHA-256 is failed.");
        printbytesln(&d[..]);
        printbytesln(&out[..]);
    }
...
fn test_sha384_inner(m: &[u8], d: &[u8]) {

    let mut out: [u8; 48] = [0; 48];

    Sha384::digest_oneshot(m, &mut out[..]).unwrap();
    if !eqbytes(d, &out[..]) {
        println!("Err: testing SHA-384 is failed.");
        printbytesln(&d[..]);
        printbytesln(&out[..]);
    }
...
fn test_sha512_inner(m: &[u8], d: &[u8]) {

    let mut out: [u8; 64] = [0; 64];

    Sha512::digest_oneshot(m, &mut out[..]).unwrap();
    if !eqbytes(d, &out[..]) {
        println!("Err: testing SHA-512 is failed.");
        printbytesln(&d[..]);
        printbytesln(&out[..]);
    }
...
fn test_sha512224_inner(m: &[u8], d: &[u8]) {

    let mut out: [u8; 28] = [0; 28];

    Sha512224::digest_oneshot(m, &mut out[..]).unwrap();
    if !eqbytes(d, &out[..]) {
        println!("Err: testing SHA-512224 is failed.");
        printbytesln(&d[..]);
        printbytesln(&out[..]);
    }
...
fn test_sha512256_inner(m: &[u8], d: &[u8]) {

    let mut out: [u8; 32] = [0; 32];

    Sha512256::digest_oneshot(m, &mut out[..]).unwrap();
    if !eqbytes(d, &out[..]) {
        println!("Err: testing SHA-512256 is failed.");
        printbytesln(&d[..]);
        printbytesln(&out[..]);
    }
